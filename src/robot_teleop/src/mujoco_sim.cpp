...
void applyEEDelta(double dx, double dy, double dz,
...
void keyboard(GLFWwindow* window, int key, int /*scancode*/, int action, int /*mods*/) {
    ...
    switch (key) {
        // Translation
        case GLFW_KEY_J: key_tx_pos = on; if (off) key_tx_pos = false; break;
        case GLFW_KEY_L: key_tx_neg = on; if (off) key_tx_neg = false; break;
        case GLFW_KEY_I: key_ty_pos = on; if (off) key_ty_pos = false; break;
        case GLFW_KEY_K: key_ty_neg = on; if (off) key_ty_neg = false; break;
        case GLFW_KEY_O: key_tz_pos = on; if (off) key_tz_pos = false; break;
        case GLFW_KEY_U: key_tz_neg = on; if (off) key_tz_neg = false; break;
        // Rotation
        case GLFW_KEY_S: key_rx_pos = on; if (off) key_rx_pos = false; break;
        case GLFW_KEY_W: key_rx_neg = on; if (off) key_rx_neg = false; break;
        case GLFW_KEY_A: key_ry_pos = on; if (off) key_ry_pos = false; break;
        case GLFW_KEY_D: key_ry_neg = on; if (off) key_ry_neg = false; break;
        case GLFW_KEY_E: key_rz_pos = on; if (off) key_rz_pos = false; break;
        case GLFW_KEY_Q: key_rz_neg = on; if (off) key_rz_neg = false; break;
        // Gripper
        case GLFW_KEY_F: key_gripper_open  = on; if (off) key_gripper_open  = false; break;
        case GLFW_KEY_H: key_gripper_close = on; if (off) key_gripper_close = false; break;
        // Change steps
        case GLFW_KEY_EQUAL:
...
int main() {
    ...
    while (!glfwWindowShouldClose(window)) {
        ...
        if (key_tx_pos) dx += EE_STEP;
        if (key_tx_neg) dx -= EE_STEP;
        if (key_ty_pos) dy += EE_STEP;
        if (key_ty_neg) dy -= EE_STEP;
        if (key_tz_pos) dz += EE_STEP;
        if (key_tz_neg) dz -= EE_STEP;

        // Rotation
        double drx = 0, dry = 0, drz = 0;
        if (key_rx_pos) drx += ROT_STEP;
        if (key_rx_neg) drx -= ROT_STEP;
        if (key_ry_pos) dry += ROT_STEP;
        if (key_ry_neg) dry -= ROT_STEP;
        if (key_rz_pos) drz += ROT_STEP;
        if (key_rz_neg) drz -= ROT_STEP;
        ...
        if (key_gripper_open)  gripper_ctrl = std::max(0.0,   gripper_ctrl - GRIPPER_STEP);
        if (key_gripper_close) gripper_ctrl = std::min(255.0, gripper_ctrl + GRIPPER_STEP);
        ...
        snprintf(info, sizeof(info),
			"Step Rates: EE_STEP (%0.5f)  ROT_STEP (%0.5f)\n"
            "Translation  I/K: X  J/L: Y  U/O: Z\n"
            "Rotation     W/S: X  A/D: Y  Q/E: Z\n"
            "Gripper      F: Open  H: Close  (%.0f)\n"
            "Time: %.2f",
            EE_STEP, ROT_STEP, gripper_ctrl, mj_data->time);
        mjr_overlay(mjFONT_NORMAL, mjGRID_BOTTOMLEFT, main_view, info, NULL, &con);
